...
async def skipto(ctx, index, bot):
    ...
    if ctx.guild.id not in song_queue:
        ...
        if ctx.guild.id not in song_queue:
            ...
            voice.stop()
            try:    embedVar = discord.Embed(title=f'我来播放这首歌了捏', description=f'{info.title}\n{info.watch_url}', color=0x8B4C39)
            except: embedVar = discord.Embed(title=f'我来播放这首歌了捏', description=f'{info["title"]}\n{info["url"]}', color=0x8B4C39)
