...
async def play(ctx, url, bot, msg = None):
    ...
    new_song_len = await addToQueue(ctx, url = url)
...
async def play_music(ctx, bot, msg, new_song_len):
    ...
    if not voice.is_playing():
        song = play_first_song(voice)
        embedVar = discord.Embed(title=f'我来播放这首歌了捏！', description=f'{song.name}\n{song.url}', color=0x8B4C39)
        await msg.edit(content='', embed=embedVar)

    elif new_song_len == 1:
        dbloader = DBloader()
        server = dbloader.load_server(ctx.guild.id)

        song = server.get_last_song()
        embedVar = discord.Embed(title=f'我把这首歌加入播放列表了捏！', description=f'{song.name}\n{song.url}', color=0x8B4C39)
        await msg.edit(content='', embed=embedVar)
