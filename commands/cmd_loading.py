...
async def save_url_to_file(ctx, url, bot):
    ...
    if "playlist?list=" not in url or not info or 'entries' not in info:
        msg.edit(embed=str_not_playlist)

    with open(f'./QueueLog/UserPlaylist/{ctx.author.id}.json', 'w') as f:
        f.write(url)

    str_save_playlist = discord.Embed(title=f'专辑{info["title"]}保存到播放列表了捏！', description="", color=0x8B4C39)
