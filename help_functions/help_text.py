...
class 下载状态(Enum):
    # 起始情形，帮助player.download_list()找出需要player.download_audio()的info
    未下载 = 0
    # 赋值：仅初始化，也就是help_info.create_info()

    # 过程情形，未开始下载，但是已经在player.download_audio()的for循环中等待下载
    待下载 = 1
    # 赋值：player.download_audio()，player.play()给random()到前排的curr_info下载
    ...
    # 失败情形，帮助player.download_audio()判断是否还需要下载，下载途中歌曲是否已被移除歌单，若下载完毕则直接删除
    放弃下载 = 4
...
str_no_playlist = discord.Embed(title=f'没有找到播放列表捏！', description='', color=FAILURE)
